  ...
  return (
    <div className="accounts-root">
      ...
      <div className="accounts-body animate-fade">
        ...
        {accounts.length === 0 ? (
          <div className="no-accounts"><span>🔍</span><p>No Options accounts found. Please create one at app.deriv.com</p></div>
        ) : (
          <>
...
function AccountCard({ account, onSelect, isLoading }: { account: DerivAccount; onSelect: (a: DerivAccount) => void; isLoading: boolean }) {
  const balance = typeof account.balance === 'number' ? account.balance.toFixed(2) : account.balance ?? '0.00';
  return (
    <div className={`account-card ${isLoading?'loading':''}`} onClick={() => !isLoading && onSelect(account)}>
      ...
      {account.balance!==undefined && <div className="acc-balance mono">{balance} {account.currency}</div>}
